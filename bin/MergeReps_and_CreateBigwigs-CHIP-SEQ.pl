     ...
     if($arr[3] eq "SINGLE-END")
     {
      ...
      if($numberoftreatmentReps > 1)
      {
       ...
       if($species eq "echinobase-pmin")
       {
        ...
        {
         my $readtype="BAM";
         system("$DNAseqmergereps", "$final_FilesMerge", "$MergedBam", "$readtype", "$final_mergedname", "$peakcalldir", "$genomesize_echinobase_pmin", "$DIR");
        }
        if($arr[3] eq "PAIRED-END")
        {
         my $readtype="BAMPE";
       ...
       if($species eq "echinobase-spur")
       {
        ...
        {
         my $readtype="BAM";
         system("$DNAseqmergereps", "$final_FilesMerge", "$MergedBam", "$readtype", "$final_mergedname", "$peakcalldir", "$genomesize_echinobase_spur", "$DIR");
        }
        if($arr[3] eq "PAIRED-END")
        {
         my $readtype="BAMPE";
