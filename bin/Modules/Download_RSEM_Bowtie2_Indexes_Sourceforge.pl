...
if($option eq "yes")
{
 print "\nDownloading RSEM indexes for ECHINOBASE PMIN and SPUR\n";
 system('wget --no-check-certificate https://sourceforge.net/projects/csbb-repository/files/RSEM_ECHINOBASE.zip');
 sleep(1);
 ...
 system('unzip RSEM_ECHINOBASE.zip');
 ...
 print "\nRSEM Idexes for Echinobase, Human, Mouse, Frog and Zebrafish are downloaded and unpacked\n";
}
if($option eq "no")
{
 print "\nCSBB is not downloading RSEM Indexes for Echinobase, human, mouse, frog and zebrafish \: Please Note CSBB will not be able to run RNA\-SEQ pipelines without the indexes\n";
 sleep(1);
...
if($option1 eq "yes")
{
 print "\nDownloading Bowtie2 indexes for ECHINOBASE\n";
 system('wget --no-check-certificate https://sourceforge.net/projects/csbb-repository/files/Bowtie_indexes_Echinobase.zip');
 sleep(1);
 ...
 sleep(1);
 print "\nNow will unzip all the indexes and prep for usuage\n";
 system('unzip Bowtie_indexes_Echinobase.zip');
 ...
 system('rm Bowtie_indexes_Echinobase.zip');
 ...
 print "\nBowtie2 Idexes for Echinobase, Human, Mouse and Frog are downloaded and unpacked\n";
}
if($option1 eq "no")
{
 print "\nCSBB is not downloading Bowtie2 Indexes for Echinobase, human, mouse and frog \: Please Note CSBB will not be able to run ChIP\/ATAC\-SEQ pipelines without the indexes\n";
 sleep(1);
 print"\nIf you wish to download then please run install module again and prompt for yes or go to https://sourceforge.net/projects/csbb-repository/files/?source=navbar for downloading indexes\n";
}